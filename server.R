...
function(input, output, session){
  ...
  observeEvent(input$submit, {
    ...
    a <- c("Type", "Latitude", "Longitude","Meteo Database", "Nominal Power(Kwp)", "Area/m2", "System Loss(%)", "Mode Trek"," Inclination Angle*")
  ...
  observeEvent(input$solaR, {
      ...
      ##Generate Site Specs
      c <- c("Site Name", "Owner","Coordinates", "Nominal Power(Kwp)", "Area/m2", "Mode Trek"," Inclination Angle")
      ...
      prod.solaR <- as.data.frameM(prod.site, complete = TRUE)
      solaR.month <- aggregate(as.zooD(prod.site), by=month, sum)
      solaR <- solaR.month$Yf*10000
      ...
      site <- data.frame("Eac" = site, "Source" = "Site")
      ...
      output$box1 <- renderValueBox({
        valueBox(
          paste0(round(sum(site$Eac)/1000, digits = 0), " GWh"), "Energy in 2020", icon = icon("solar-panel"),
      ...
      output$box2 <- renderValueBox({
        valueBox(
          paste0(round(sum(site$Eac)*100/sum(solaR$Eac), digits = 1), " %"), "compared with solaR simulation result", icon = icon("thumbs-up"),
          color = "green", width = 10
        )
      })
      
      
      sol.data <- read.table("https://api.solcast.com.au/world_radiation/forecasts?latitude=37.46&longitude=-84.43&format=csv&api_key=YXObq8uGpoS41MsLrOhyla0ofzxtB-LI",
      ...
      output$predication <- renderPlotly({
        ...
        fig1 <- plot_ly(site.hour, x = ~Date)
        fig1 <- fig1 %>% add_lines(y = ~ G0, name = "GHI(G0)", yaxis = "y",
                                   line = list(color = 'rgb(255, 127, 14)', width = 2))
        fig1 <- fig1 %>% add_lines(y = ~ Ta, name = "Temperature", yaxis = "y2",
                                   line = list(color = 'rgb(31, 119, 180)', width = 2))
      ...
      output$predication <- renderPlotly({
        ...
        fig1 <- fig1 %>% add_lines(y = ~ G0.y, name = "Projected GHI(G0)", yaxis = "y",
                                   line = list(color = 'rgb(255, 127, 14)', width = 1, dash = 'dash'))
        ...
        fig1 <- fig1 %>% add_lines(y = ~ Ta.y, name = "Projected Temperature", yaxis = "y2",
                                   line = list(color = 'rgb(44, 160, 44)',width = 1, dash = 'dash'))
        ...
        fig2 <- fig2 %>% add_lines(y = ~Kw.y, name = "Projected Pac",
                                   line = list(color = 'rgb(31, 119, 180)', width = 2, dash = 'dash'))
        fig2 <- fig2 %>% layout(
          xaxis = list(title = ""),
